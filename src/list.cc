...
bf_pluralize(Var arglist, Byte next, void *vdata, Objid progr)
{
    ...
    if (quantity == 1) {
        for (const auto& pair : IRREGULAR_SINGULAR_RULES) {
            if (token == pair.first) {
                r.v.str = str_dup(arglist.v.list[1].v.str);
                free_var(arglist);
                return make_var_pack(r);
        ...
        for (const auto& pair : IRREGULAR_PLURAL_RULES) {
            if (token == pair.first) {
                r.v.str = str_dup(pair.second.c_str());
                free_var(arglist);
                return make_var_pack(r);
            }
        }
...
{   /* (subject, targets [, keys]) */
    Objid result = match_object(69, arglist.v.list[1].v.str);
    Var results = new_list(1);
    results.v.list[1] = Var::new_obj(result);
    Var keys = new_list(0);
    ...
    for (int i=0;i < matches.size();i++) {
        r = listappend(r, var_ref(arglist.v.list[2].v.list[matches[i] + 1]));
    }
    free_var(keys);
    free_var(arglist);
    return make_var_pack(r);
